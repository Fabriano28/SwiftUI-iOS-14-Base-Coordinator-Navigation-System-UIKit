...
final class AppStateManager: ObservableObject {
    @Published private(set) var currentState: AppState = .unauthenticated
    @Published private(set) var transitionDirection: TransitionDirection = .forward
    
    func setState(to newState: AppState) {
        // Determine direction by comparing old and new states for animations
        ...
        // Wrap the state change in an animation
